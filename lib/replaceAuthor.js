        ...
        .then((profiles) => {
          ...
          if(!!users[0]&&!!profiles[0]&&!!entities[0]){
            namedEntities = entities.map((entity) => {
              ...
              if(!!entity.reciever){
                recieverUser = users.filter(u => u._id == entity.reciever)[0]
                recieverProfile = profiles.filter(p => p.userId == recieverUser._id)[0]
                newEntity.recieverName = (!!recieverProfile?recieverProfile.fullName:recieverUser.email.split('@')[0])
              }
              return newEntity
            })
          }
